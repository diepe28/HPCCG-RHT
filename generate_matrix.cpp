...
void generate_matrix(int nx, int ny, int nz, HPC_Sparse_Matrix **sparseMatrix, double **x, double **b, double **xexact) {
    ...
    *sparseMatrix = new HPC_Sparse_Matrix; // Allocate matrix struct and fill it
    ...
    long local_nrow = nx * ny * nz; // This is the size of our subblock
    assert(local_nrow > 0); // Must have something to work with
    long local_nnz = 27 * local_nrow; // Approximately 27 nonzeros per row (except for boundary nodes)

    long total_nrow = local_nrow * size; // Total number of grid points in mesh
    long long total_nnz =
            27 * (long long) total_nrow; // Approximately 27 nonzeros per row (except for boundary nodes)

    long start_row = local_nrow * rank; // Each processor gets a section of a chimney stack domain
    long stop_row = start_row + local_nrow - 1;
