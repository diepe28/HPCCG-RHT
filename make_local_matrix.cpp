...
void make_local_matrix(HPC_Sparse_Matrix * A) {
    ...
    struct timespec newStart, newEnd;
    double elapsedTime;
    ...
    if (debug) clock_gettime(CLOCK_MONOTONIC, &newStart); //t0 = mytimer();
    ...
    if (debug) {
        GetTimeSince(newStart, t0 = ) //t0 = mytimer() - t0;
    ...
    clock_gettime(CLOCK_MONOTONIC, &newStart); //t0 = mytimer();
